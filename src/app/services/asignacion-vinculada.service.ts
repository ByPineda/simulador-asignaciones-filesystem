...
export class AsignacionVinculadaService {
  ...
  llenarGrafica() {
    ...
    this.listaArchivos.forEach((archivo) => {
      for (let index = 0; index < archivo.bloques.length; index++) {
        this.bloques[archivo.bloques[index]].nombre = archivo.nombre;
        this.bloques[archivo.bloques[index]].color = archivo.color;
        this.bloques[archivo.bloques[index]].occupied = true;
        if (archivo.bloques[index + 1] != null) {
          this.bloques[archivo.bloques[index]].siguiente_bloque =
            this.bloques[archivo.bloques[index + 1]];
        }
      }
    });
  }
