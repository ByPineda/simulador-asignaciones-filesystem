...
export class ValidadorService {
  public validarNombre(lista: any[], archivo: Archivo) {
  ...
  public validarInicio(lista: any[], archivo: Archivo) {
    if(archivo.inicio >= 0){
      ...
      if (archivo.inicio == 0) {
        ...
        if (lista.filter((arch) => arch.inicio === archivo.inicio).length == 0) {
  ...
  public validarColor(archivo: Archivo){
    if(archivo.color != ''){
      return true;
    }
    else{
